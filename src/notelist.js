...
const insertNoteContentIntoActiveEditor = common.insertNoteContentIntoActiveEditor;
const generateHash = common.generateHash;
const getTitleFromText = common.getTitleFromText;
...
function getNoteList(context) {
    ...
    const htmlContent = getHtmlViewContent();
 ...
 * @param {vscode.ExtensionContext} context
 */
function initNoteList(context) {
    if (!notelist || isDispose) {
        ...
        notelist.webview.onDidReceiveMessage(
            (message) => {
                console.log(message);
                let notes = context.globalState.get(
                    "coolwriter.notes",
                    []
                );
                    ...
                    case "searchNotes":
                        ...
                        notelist.webview.postMessage({
                            ...
                            notes: notes.filter((note) =>
                                note.content.includes(message.query)
                            ),
                        });
                        break;
                    case "deleteNote":
 ...
 * @param {vscode.ExtensionContext} context
 */
async function notelistCommand(context) {
    initNoteList(context);
}

/**
 * @param {vscode.ExtensionContext} context
 */
async function addNoteCommand(context) {
    initNoteList(context);
